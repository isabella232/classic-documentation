...
Looking for information on how to retrieve content-enabled DCP attributes via JavaScript methods?  Head on over to the [Optimizely X documentation](/x/solutions/javascript/reference/index.html#function_dcp)!
...
Sends tracking events even if client is in preview or `force_variations`/`force_audiences` are present. Does NOT override disable tracking API.

You can do this by using the query parameter `optimizely_force_tracking=true`.
